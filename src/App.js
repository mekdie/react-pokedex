  ...
  async function evolutionObj(data) {
    return {
      ...
      imageUrl: `https://assets.pokemon.com/assets/cms2/img/pokedex/full/${data.id
        .toString()
        .padStart(3, "0")}.png`,
      pixelImage: await fetch(`https://pokeapi.co/api/v2/pokemon/${data.id}`)
        .then((res) => res.json())
        ...
        ? evolution_data.chain.evolves_to[0].evolves_to.length !== 0
          ? evolution_data.chain.evolves_to[0].evolves_to.length >= 1
            ? //multiple evolution fn, else single evolution (direct url)
              multipleEvolvesURL(evolution_data.chain.evolves_to[0].evolves_to)
            : evolution_data.chain.evolves_to[0].evolves_to[0].species.url
          : false
        : false;
    ...
    <>
      ...
      {!loading && (
        <>
          ...
          <Routes>
            <Route path="/" element={<Home {...homeProps} />}></Route>
            <Route
              path="/about"
              element={<About inAboutPage={(el) => setAboutPage(el)} />}
            />
            <Route
              path="/search"
              element={
            ...
            <Route
              ...
              element={
                <PokemonInfo
                  data={pokemonInfo}
                  fetchPokemonInfo={(childID) => fetchPokemonInfo(childID)}
                  inPokemonPage={(el) => setPokemonInfoPage(el)}
            ...
            <Route
              ...
              element={
                <NotFound
                  ...
                  setPokemonInfoPage={(e) => setPokemonInfoPage(e)}
                />
              }
            ></Route>
          </Routes>
